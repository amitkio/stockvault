...
portfolio_bp_single = Blueprint("portfolio_bp_single", __name__, url_prefix="/portfolio")
...
def create_portfolio():
    ...
    existing_portfolio = db.session.execute(db.select(Portfolio).filter_by(user_id=user_id)).scalar_one_or_none()
    if existing_portfolio:
        return jsonify({"message": "User already has a portfolio."}), HTTPStatus.CONFLICT
...
def post_transaction(portfolio: Portfolio):
    ...
    if not all([symbol, quantity_str, tx_type_str]):
        return jsonify({"message": "Missing required fields: symbol, quantity, transaction_type"}), HTTPStatus.BAD_REQUEST
    ...
    try:
        ...
        return jsonify({"message": "Invalid quantity provided."}), HTTPStatus.BAD_REQUEST

    try:
        tx_type = TransactionTypeEnum[tx_type_str]
    except KeyError:
        return jsonify({"message": "Invalid transaction_type. Must be 'BUY' or 'SELL'."}), HTTPStatus.BAD_REQUEST

    # --- Database Operations ---
    stock = db.session.execute(db.select(Stock).filter_by(symbol=symbol)).scalar_one_or_none()
    if not stock:
        return jsonify({"message": f"Stock with symbol '{symbol}' not found."}), HTTPStatus.NOT_FOUND

    try:
        # Use the service to execute the transaction
        transaction = execute_transaction(db.session, portfolio, stock, quantity, tx_type)
        db.session.commit()
